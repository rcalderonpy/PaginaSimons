...
class VentaCabRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    ))
    {
        ...
        $query = $em->createQuery('SELECT v 
          FROM ContabilidadBundle:VentaCab v 
          WHERE v.cliente=?1 AND MONTH(v.fecha)=?2 AND YEAR(v.fecha)=?3');
    ...
    {
        $em=$this->getEntityManager()->getRepository('ContabilidadBundle:VentaCab');

        $query=$em->createQueryBuilder('v')->select('count(1) as cantidad, sum(v.g10+v.g5+v.iva10+v.iva5+v.exe) as suma')
//            ->from('VentaCab.php', 'v')
            ->Where("v.cliente = :cliente")
